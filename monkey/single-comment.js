...
(function () {
    ...
    // 全局变量
    // --------------------------------------------------------------------------------
    var isMobile = window.location.host === 'm.weibo.cn', //是移动端链接还是pc端链接
        curTotal = - 1, // 当前评论总数
        textList; // 根据微博正文匹配的文案数组
    ...
    // 页面 DOM 元素
    var pageElems = {
        weiboText: null, //微博正文容器
        qtyBar: null, //显示点赞评的工具栏
        list: null, //评论列表
        listLoading: null,//评论列表中的loading元素
        input: null,//填写评论的文本域
        submitBtn: null, //提交评论的按钮
    };
    ...
    function floatCopy () {
        ...
        setTimeout(function () {
            floatElems.copyInput.value = getText();
    ...
    function appendData () {
        ...
        floatElems.status.innerText = '状态';
        ...
        floatElems.copyBtn.innerText = '复制';
        floatElems.copyBtn.onclick = floatCopy;
